    ...
    <ul class="flex flex-col mt-3 list-none">
      ...
      >
        <div class="flex items-center">
          <span :class="`bg-${filter.color}`" class="inline-block w-2 h-2 mr-2 rounded-full" /> {{ filter.label }}
        </div>
        <span :class="filter.active ? `text-${filter.color}` : 'text-brand-darkgray'" class="font-bold">
          {{ filter.amount }}
        </span>
      </li>
    </ul>
  ...
  async setup(props, { emit }) {
    ...
    function handlerSelect({ type }) {
      ...
      state.filters = state.filters.map(filter => {
        return { ...filter, active: filter.type === type };
      });

      emit('select', type);
    }

    return { store, state, handlerSelect };
  }
};
</script>
